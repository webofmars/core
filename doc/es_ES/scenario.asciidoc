...
* *Ver variables*: permite ver las variables, sus valores y el lugar donde se utilizan
...
* *Avanzado*: permite hacer todo, pero se necesita un mayor conocimiento
...
* *Ejecutar en primer plano*: lanza la secuencia de comandos directamente y no en un hijo ejecución separada. Tenga en cuenta que esta opción puede ser peligrosa porque pondrá en pausa cualquier otra acción de la escena. Nunca active una escena lanzada mediante programación horaria o calendario
...
* botón para exportar una escena en modo texto (útil para mostrar en el foro o al equipo de soporte)
* un botón para ver el log de una escena
* un botón para duplicar la escena

Abajo a la izquierda encontrará la configuración de la escena: 

* *Escena*: para indicar si la escena se programará (en un momento y fecha y precisos) o si será provocada por un evento (como por ejemplo la detección de un movimiento)
* *Ejecución*: en modo programado permite seleccionar si se va a ejecutar una sóla vez o de forma repetida
* *A partir de*: Si no se ejecuta una vez, debe especificar la fecha y la hora
...
* *Estado*: Seleccione el estado que debe tener el comando anterior para que se ejecute la acción (en función del comando las opciones pueden cambiar)

Finalmente, a la derecha encontrará las acciones, simplemente basta con hacer clic en agregar una acción

=== Ejemplo de luz activada por presencia

image::../images/scenario13.JPG[]

Éste es un script muy sencillo que enciende la luz al detectar una presencia
